...
function StreamSection({ setSnackbar }) {
  ...
  const [studentInfo, setStudentInfo] = useState(null);
    ...
    const fetchStudentInfo = async () => {
      try {
        const data = await apiRequest('/api/parent/student-info');
        setStudentInfo(data);
      } catch (err) {
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      try {
        ...
        // Filter announcements based on student's age group and CDC
        const filteredAnnouncements = data.announcements.filter(ann => {
          // Check if announcement is for all ages or matches student's age group
          const ageMatch = ann.age_filter === 'all' || ann.age_filter === studentInfo.ageGroup;
          
          // Check if announcement is for all CDCs or matches student's CDC
          const cdcMatch = !ann.cdc_id || ann.cdc_id === studentInfo.cdc_id;
          
          return ageMatch && cdcMatch;
        });
